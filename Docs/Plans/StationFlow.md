...
| 에스컬레이터 애니메이션 위로 행인 이동 | 절반 | 경사로 셀은 걷지만 애니메이션 연동 없음 |
...
아트의 `BP_escalator_up_s`/`down_s`(스켈레탈 `AC_001` + `AC_SCALE_ANIM_Anim`)는 그대로 레벨에 놓는다.
행인은 경사로 셀을 걷는다(그리드가 경사면을 굽는다, `MaxSlopeAngle` 45 이상 필요 — 아트 에스컬레이터는
4x8x8 m로 정확히 45°이므로 테스트 맵은 50으로 둔다). 스텝 애니메이션 속도와 행인 `MoveSpeed`를 눈으로
맞추는 것까지만. 콜리전 프리셋이 트레이스에 잡히는지 임포트 후 확인(7절 Q6).
