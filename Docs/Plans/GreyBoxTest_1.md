...
주요 동선 **F0 → 계단(Stiar_B1_001) → B1 → 개찰구 → 에스컬레이터 → B2 승강장 남쪽**은 겹침이 없어 단일 그리드로 이동 가능하다.
...
1. World Settings → World Partition → **Enable Streaming 끄기** (수동).
2. 아트 수정 요청: TicketGate_B1_005~008 y −0.18 m, Stiar_B1_002 y −0.212 m 스냅. 반영되면 `export_greybox.py` 재실행 → 해당 SM 재임포트.
3. 슬로프/클리어런스로 막힌 35셀이 어디인지 `bDrawGridInEditor`로 확인하고 필요하면 마커로 보정.
4. 2차: 층별 그리드 + 링크 셀 (6절).
